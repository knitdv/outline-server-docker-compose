...
function display_usage() {
  cat <<EOF
Usage: install_server.sh [--hostname <hostname>] [--api-port <port>] [--keys-port <port>] [--data-dir <data dir>]
...
function create_persisted_state_dir() {
  ...
  readonly STATE_CONTAINER_DIR="${SHADOWBOX_CONTAINER_DIR}/persisted-state"
  mkdir -p "${STATE_DIR}"
...
#измененная функция для создание docker-compose файла
...
function parse_flags() {
  local params
  params="$(getopt --longoptions hostname:,api-port:,keys-port:,data-dir: -n "$0" -- "$0" "$@")"
  eval set -- "${params}"
